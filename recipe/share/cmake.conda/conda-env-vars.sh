...
#
# CONDA_CMAKE_SYS_ISOLATION_FLAGS
...
#
# CONDA_CMAKE_DEFAULT_FLAGS (CONDA_CMAKE_SYS_ISOLATION_FLAGS+CONDA_CMAKE_PLATFORM_FLAGS) always
# and ..
# CONDA_CMAKE_TOOLCHAIN_FLAGS when it does not cause breakage (it will improve isolation from the system and save time)
...
CONDA_CMAKE_SYS_ISOLATION_FLAGS+=(-DCMAKE_FIND_LIBRARY_CUSTOM_LIB_SUFFIX=)
CONDA_CMAKE_SYS_ISOLATION_FLAGS+=(-DFIND_LIBRARY_USE_LIB32_PATHS=NO)
CONDA_CMAKE_SYS_ISOLATION_FLAGS+=(-DFIND_LIBRARY_USE_LIB64_PATHS=NO)
CONDA_CMAKE_SYS_ISOLATION_FLAGS+=(-DFIND_LIBRARY_USE_LIBX32_PATHS=NO)

declare -a CONDA_CMAKE_TOOLCHAIN_FLAGS
CONDA_CMAKE_TOOLCHAIN_FLAGS=(-DCMAKE_TOOLCHAIN_FILE="$(_get_sourced_dirname)/conda-${target_platform}.toolchain.cmake")
...
declare -a CONDA_CMAKE_DEFAULT_FLAGS
CONDA_CMAKE_DEFAULT_FLAGS=("${CONDA_CMAKE_SYS_ISOLATION_FLAGS[@]}" "${CONDA_CMAKE_PLATFORM_FLAGS[@]}")
CONDA_CMAKE_DEFAULT_FLAGS+=(-DCMAKE_FIND_ROOT_PATH="${PREFIX}")
CONDA_CMAKE_DEFAULT_FLAGS+=(-DCMAKE_INSTALL_RPATH="${PREFIX}/lib")
CONDA_CMAKE_DEFAULT_FLAGS+=(-DCMAKE_INSTALL_PREFIX="${PREFIX}")

if [[ ${DEBUG_C} == yes ]]; then
  CONDA_CMAKE_DEFAULT_FLAGS+=(-DCMAKE_BUILD_TYPE:STRING=Debug)
else
  CONDA_CMAKE_DEFAULT_FLAGS+=(-DCMAKE_BUILD_TYPE:STRING=Release)
fi
